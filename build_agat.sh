...
export INITRAMFS_SOURCE="${KERNELDIR}/../boot.img-ramdisk"
export INITRAMFS_TMP="/tmp/initrams-tmp"
export RELEASEDIR="${KERNELDIR}/../releases"
export CCACHE_DIR="/home/jdsingle76/.ccache"
export USE_CCACHE=1
...
# Target Configs ...
export AGAT_DEFCONFIG=jdsingle76_defconfig
...
#
export ARCH=arm
CROSS_COMPILE=/home/jdsingle76/Kernel/kernel-extras/arm-eabi-4.6/bin/arm-eabi-
# Choose Propper Compiler setup
if [ "${USE_CCACHE}" == "1" ];
then
echo -e "${TXTGRN}Using ccache Compiler Cache ..${TXTCLR}"
export CCACHE_DIR="/home/jdsingle76/.ccache"
export CROSS_COMPILE=/home/jdsingle76/Kernel/kernel-extras/arm-eabi-4.6/bin/arm-eabi-
else
...
then
  if [ ! -f $KERNELDIR/arch/arm/configs/$AGAT_DEFCONFIG ];
  then
    clear
    echo -e "  "
    echo -e "${BLDRED}Error: can not find default Kernel Config: ${AGAT_DEFCONFIG} !${TXTCLR}"
  ...
  echo -e "${TXTYLW}Creating Kernel config from default: ${AGAT_DEFCONFIG} ${TXTCLR}"
  make ARCH=arm ${AGAT_DEFCONFIG} 
...
## Build zImage
nice -n 10 make -j8 CC="ccache /home/jdsingle76/Kernel/kernel-extras/arm-eabi-4.6/bin/arm-eabi-gcc" zImage 2>&1 | tee compile-zImage.log
...
## Build Modules
nice -n 10 make -j8 CC="ccache /home/jdsingle76/Kernel/kernel-extras/arm-eabi-4.6/bin/arm-eabi-gcc" modules 2>&1 | tee compile-modules.log
